...
'''
def fetch_headlines(page, site):
    page.goto(site['url'], wait_until="domcontentloaded", timeout=90000)
    sel = site['selector']
    page.wait_for_load_state("networkidle")

# Trigger lazy loading for scroll-based sites
    page.mouse.wheel(0, 5000)
    page.wait_for_timeout(1500)

    page.wait_for_selector(sel['container'], timeout=20000)
    stories = page.locator(sel['container'])

    results = []

    for i in range(min(15, stories.count())):
        story = stories.nth(i)

        try:
            headline = story.locator(sel['headline']).inner_text().strip()
        except:
            headline = ""
        ...
        if sel['description']:
            try:
                description = story.locator(sel['description']).inner_text().strip()
            except:
                pass

        if headline:
            results.append((headline, description))
